...
export default {
  name: "index",
    ...
    //   $route: {
    //     handler() {
    //       this.scene_id = this.$route.query.id;
    //        location.reload();
    //       this.previewClick(this.scene_id);
  ...
  data() {
    return {
      ...
      top_menu: { widgetCode: {} },//顶部菜单下方模块-针对图书期刊频道
      temp_list: [],
    };
  },
  mounted(){
  ...
  methods: {
    ...
    isDivShow(val){
        var is_show = true;
        if(val == 'unified_retrieval_sys_temp3'){
            is_show = false;
        }else if(val == 'unified_retrieval_sys_temp4'){
            is_show = false;
        }
        return is_show;
    },
